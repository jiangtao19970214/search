...
<!--        直接返回最初界面
        @@@@@已实现-->
        <a href="../index.html" class="box" ><i class="iconfont icon-cangku"></i><span></span></a>
<!--        黑白显示框事件
            @@@@@已实现-->
...
<%--        显示数据项、数据项后加button进行删除与更新--%>
        <table>
            <caption>Film information Management</caption>
            <thead><tr><td>Movie_name</td><td>Movie_author</td><td>Movie_link</td><td>Movie_introduction</td></tr></thead>
            <%
                List<Movie> movie = (List<Movie>)request.getAttribute("movie_message");
                for (Movie movie1 : movie) {

            %><tr><td><%=movie1.getMovie_name()%><td><%=movie1.getMovie_author()%><td><%=movie1.getMovie_link()%><td><%=movie1.getMovie_introduction()%>
            <%
                }
            %>
        </table>
