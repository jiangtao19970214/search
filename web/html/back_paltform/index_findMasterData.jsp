...
<%--        显示数据项、数据项后加button进行删除与更新--%>
        <table>
            <caption>Administrator Information Management</caption>
            <thead><tr><td>UserName</td><td>Number</td><td>PassWord</td></tr></thead>
            <%
                List<Master> master = (List<Master>)request.getAttribute("master_message");
                for (Master master1 : master) {
            %><tr><td><%=master1.getUsername()%><td><%=master1.getCount()%><td><%=master1.getPassword()%>
            <%
                }
            %>
