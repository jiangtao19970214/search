...
<%--        显示数据项、数据项后加button进行删除与更新--%>
        <table>
            <caption>User Information Management</caption>
            <thead><tr><td>UserName</td><td>PassWord</td><td>Email</td></tr></thead>
            <%
                List<User> user = (List<User>) request.getAttribute("user_message");
                for (User user1 : user) {

                %><tr><td><%=user1.getUser_name()%><td><%=user1.getPassword()%><td><%=user1.getEmail()%>
            <%
                }
            %>
        </table>
